...
from transformers import BertModel, BertConfig, BertTokenizer, TFBertModel, TFBertForPreTraining
...
import BERT.BERTmodeling


import DNABERT.motif.motif_utils
import DNABERT.src.transformers.modeling_tf_bert


# to see docker if running > docker ps
# to stop docker container: docker stop rnashapes
# 1. docker restart rnashapes                (starts container)
# 2. docker exec -it rnashapes echo "hello"  (run command)


#### python ideeps.py --train=True --data_file=1_PARCLIP_AGO1234_hg19/30000/training_sample_0/sequences.fa.gz --model_dir=models
#### python ideeps.py --train=True --data_file=2_PARCLIP_AGO2MNASE_hg19/30000/training_sample_0/sequences.fa.gz --model_dir=models
            ...
            if line[0] == '>':
                ...
                if len(seq):
                    if struct_exist:
                        ...
                        fw.write(old_name)
                        seq_array, struct = get_RNA_structure_concolutional_array(seq, fw, structure = None)
                        ###seq_array, struct = get_RNA_structure_concolutional_array(seq, None)
                    seq_list.append(seq_array)
...
def get_transformers_network():
    ...
    # pdb.set_trace()
    model = Sequential()
    #print("seq", seq_model.shape)
    #print("struct", struct_model.shape)
    #print("BEFORE CONCATENATE")
    #concatted = Concatenate(axis=1)

    #concatted = Concatenate()([seq_model, struct_model])
    model.add(concatenate([seq_model, struct_model], -1))
    #model.add(Merge([seq_model, struct_model], mode='concat', concat_axis=1))

    #print("CONCATENATED MODEL SHAPE")
    ...
    #### TENSORFLOW HUB ATTEMPT ###
    #BERT_MODEL_HUB = "https://tfhub.dev/tensorflow/bert_en_uncased_L-12_H-768_A-12"
    #test_bert = hub.load(BERT_MODEL_HUB, trainable=True)
    #hub_layer = hub.keras_layer(BERT_MODEL_HUB, trainable=True)
    #model.add(hub_layer)



    ###### OR TRY DNABERT #####
    config = BertConfig.from_json_file('./4-new-12w-0/config.json')
    token_config = BertConfig.from_json_file('./4-new-12w-0/tokenizer_config.json')
    tokenizer = BertTokenizer('./4-new-12w-0/vocab.txt')
    dnabert_layer = TFBertModel.from_pretrained("./4-new-12w-0/pytorch_model.bin", from_pt=True, config=config)

    #inputs = keras.Input((None, units))


#Layers in a Sequential model should only have a single input tensor,
    # but we receive a <class 'tuple'> input: (<tf.Tensor 'IteratorGetNext:0' shape=(50, 111, 4) dtype=float32>,
    # <tf.Tensor 'IteratorGetNext:1' shape=(50, 111, 6) dtype=float32>)

    #model2 = dnabert_layer()

    #model.add(dnabert_layer)
...
def get_cnn_network():
    ...
    #print("MODEL INPUT SHAPE JUST BEFORE CNN: ")
    #print(model.output_shape)

    model.add(Bidirectional(LSTM(2*nbfilter)))

    #print("MODEL OUTPUT SHAPE FROM CNN")
    #print(model.output_shape)
...
def get_struct_network():
    ...
    #model.add(Convolution1D(filters=nbfilter, kernel_size=10, input_dim=6,
    #                        input_length=111, padding="valid"))

    model.add(Convolution1D(filters=nbfilter, kernel_size=10, input_shape=(111, 6), padding="valid"))
...
#### docker run -v $(pwd):/home/meme --user $(id -u):$(id -g) memesuite/memesuite -dist pearson -thresh 0.05 -eps -oc %s/tomtom %s/filters_meme.txt %s' % (out_dir, out_dir, 'Ray2013_rbp_RNA.meme')

    docker_start = "docker restart memesuite"
    sp.check_output(docker_start, shell=True)

    subprocess.call('docker exec -it tomtom -dist pearson -thresh 0.05 -eps -oc %s/tomtom %s/filters_meme.txt %s'
...
def run_predict():
    ...
    fw = open('result_file_struct_auc', 'wt')
...
def run_network_new(model, total_hid, training, y, validation, val_y, batch_size=50, nb_epoch=30):
    ...
    model_inputs = tf.keras.Input(shape=model.ouput_shape)
    x = tf.keras.layers.Dense(2, input_shape=(total_hid,), activation='softmax')(model_inputs)
    new_model = tf.keras.models.Model(inputs=model_inputs, outputs=x)
